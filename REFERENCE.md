...
**Font fallback**: starship's default icons (git branch, etc.) need a [Nerd Font](https://www.nerdfonts.com/) installed and selected in your terminal — without one they render as garbled/missing glyphs. `bashrc` checks `fc-list` for one at shell startup; if none is found, it points `STARSHIP_CONFIG` at `etc/starship-no-nerd-font.toml` instead, which uses plain Unicode/ASCII everywhere. This checks whether a Nerd Font is *installed*, not whether the terminal is actually configured to use it — but that lines up with the real-world condition it's meant to handle: install a Nerd Font when you're on an online machine and can, skip it when you're offline and can't, and either way the prompt renders cleanly instead of showing broken glyphs. Note: starship's own official `no-nerd-font` preset (`starship preset no-nerd-font`) has a gap — it doesn't override the *default* `git_branch` symbol, which is itself a Nerd-Font-exclusive glyph (`U+E0A0`), so `etc/starship-no-nerd-font.toml` adds that override by hand on top of the official preset.
